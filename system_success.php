<?php
	include 'connect.php';
	//Start Missing 
	$tot_m=mysql_query("SELECT COUNT(id) AS numrows FROM missing_person_info WHERE situation='missing'");
	$aa=mysql_fetch_array($tot_m);
	
	$tot_mnum=$aa['numrows'];
	
	$found_m=mysql_query("SELECT COUNT(id) AS numrows1 FROM missing_person_info WHERE situation='missing' AND found='1'");
	...
	//Start Finding
	$tot_f=mysql_query("SELECT COUNT(id) AS numrows FROM missing_person_info WHERE situation='finding'");
	$aa=mysql_fetch_array($tot_f);
	
	$tot_fnum=$aa['numrows'];
	
	$found_f=mysql_query("SELECT COUNT(id) AS numrows1 FROM missing_person_info WHERE situation='finding' AND found='1'");
	$bb=mysql_fetch_array($found_f);
	
	$found_fnum=$bb['numrows1'];
	$per_f=number_format((($found_fnum/$tot_fnum)*100),2);
	$per_fn=100-$per_f;
	//End Finding
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        var data = google.visualization.arrayToDataTable([
          ['Task', 'Success Ratio'],
          ['Missing Person Found',    <?php echo $per_found; ?>],
          ['Person Not Found',    <?php echo $per_mis; ?>] 
        ]);

        var options = {
          title: 'Missing Person Statistics'
        };
...
<script type="text/javascript" src="https://www.google.com/jsapi"></script>
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        var data = google.visualization.arrayToDataTable([
          ['Task', 'Success Ratio'],
          ['Missing Person Found',    <?php echo $per_f; ?>],
          ['Person Not Found',    <?php echo $per_fn; ?>] 
        ]);

        var options = {
          title: 'Finding Person Statistics'
        };
